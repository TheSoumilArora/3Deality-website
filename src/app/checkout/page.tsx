...
import { ArrowLeft,Truck,CreditCard,ShoppingCart,Loader2,Percent, } from "lucide-react"
...
export default function Checkout () {
  ...
  const applyCode = async () => {
    ...
    setPB(true)
    try {
      await medusa.carts.update(cart.id, { discounts: [{ code: code.trim() }], })
  ...
  const pay = async() => {
    ...
    try {
      ...
      const options = {
        key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID!,  // from Vite env
